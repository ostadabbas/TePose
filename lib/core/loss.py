...
class TePoseLoss(nn.Module):
    def __init__(
        ...
        super(TePoseLoss, self).__init__()
    ...
    def forward(
        ...
        pred_motion = torch.cat((pre_mosh,torch.unsqueeze(total_predict_thetas.mean(dim=1),1)), dim=1)
        pred_motion = torch.cat((pred_motion[:sample_2d_count], pred_motion[sample_2d_count:][~w_smpl[::2]]), dim=0)

        real_motion = data_motion_mosh['theta']
        real_motion = torch.cat((real_motion[:sample_2d_count], real_motion[sample_2d_count:][~w_smpl[::2]]), dim=0)

        if pred_motion.shape[0]>0:
            ...
            gen_loss = torch.stack(list(loss_dict.values())).sum()
            motion_dis_loss = torch.zeros(1).float()
    ...
    def keypoint_loss(self, pred_keypoints_2d, gt_keypoints_2d, openpose_weight, gt_weight):
        ...
        if len(gt_keypoints_2d) > 0:
            ...
            return torch.mean(torch.FloatTensor(1).fill_(0.)).to(self.device)

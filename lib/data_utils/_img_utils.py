...
def split_into_videos(vid_names, seqlen, stride, vidlen):
    ...
    for idx in range(len(video_names)):
        ...
        if indexes.shape[0] < seqlen:
            ...
            start_finish = [indexes[0], indexes[-1]]
            start_finish[1] = min(start_finish[1],start_finish[0]+vidlen-1)
            video_lens.append(start_finish[1]-start_finish[0]+1)
            video_start_end_indices += start_finish

    return video_start_end_indices, video_lens


def split_into_videos_val(vid_names, seqlen, stride):
    video_start_end_indices = []
...
def combine_into_chunks(vid_names, seqlen, vidlen):
    video_start_end_indices = []
    ...
    extra_len = []
    for idx in range(len(video_names)):
        extra_len.append(indices[idx][-1]-indices[idx][0]+1-seqlen+1)

    len_tmp = 0
    index_tmp = []
    video_start_end_indices = []
    for idx in range(len(video_names)):
        len_tmp = len_tmp + extra_len[idx]
        if len_tmp<vidlen-seqlen+2:
            index_tmp.append([indices[idx][0], indices[idx][-1]])
        else:
            if idx==0:
                index_tmp.append([indices[idx][0], indices[idx][0]+vidlen-1])
            else:
                video_start_end_indices.append(index_tmp)
                index_tmp = []
                len_tmp = extra_len[idx]
                if len_tmp<vidlen-seqlen+2:
                    index_tmp.append([indices[idx][0], indices[idx][-1]])
                else:
                    index_tmp.append([indices[idx][0], indices[idx][0]+vidlen-1])

...
def compute_accel(joints, vidlen_each, seqlen):
    """
    ...
    acceleration_normed = np.mean(np.linalg.norm(acceleration, axis=3), axis=2)

    output = np.zeros(1)
    for i in range(acceleration_normed.shape[0]):
        output += np.sum(acceleration_normed[i,seqlen-1:int(vidlen_each[i])-2])

    return (output/(torch.sum(vidlen_each)-vidlen_each.shape[0]*(seqlen+1)+1e-8))[0]


def compute_error_accel(joints_gt, joints_pred, vidlen_each, seqlen, vis=None):
    """
    ...
    normed = np.mean(normed[new_vis], axis=2)

    output = np.zeros(1)
    for i in range(normed.shape[0]):
        output += np.sum(normed[i,seqlen-1:int(vidlen_each[i])-4])

    return (output/(torch.sum(vidlen_each)-vidlen_each.shape[0]*(seqlen+3)+1e-8))[0]


def compute_error_accel_eval(joints_gt, joints_pred, vis=None):
    """
    Computes acceleration error:
        1/(n-2) \sum_{i=1}^{n-1} X_{i-1} - 2X_i + X_{i+1}
    Note that for each frame that is not visible, three entries in the
...
def compute_error_verts(pred_verts, target_verts=None, target_theta=None, device='cpu'):
    """
    ...
    if target_verts is None:
        ...
        betas = target_theta[:,75:].to(device)
        pose = target_theta[:,3:75].to(device)
        ...
        for b,p in zip(b_,p_):
            ...
            target_verts.append(output.vertices.detach())

        target_verts = torch.cat(target_verts, dim=0)

    assert len(pred_verts) == len(target_verts)
    error_per_vert = torch.sqrt(torch.sum((target_verts - pred_verts) ** 2, dim=2)).to('cpu').numpy()

...
from lib.data_utils._img_utils import normalize_2d_kp, transfrom_keypoints, split_into_videos_val, get_single_image_crop


logger = logging.getLogger(__name__)


class Human36M_VAL(Dataset):
    def __init__(self, load_opt, set, seqlen, vidlen, overlap=0., folder=None, debug=False):
        ...
        db_file = osp.join(TePose_DB_DIR, 'h36m_test_front_25fps_tight_db.pt')
        if osp.isfile(db_file):
            self.db = joblib.load(db_file)
        else:
            raise ValueError(f'{db_file} do not exists')

        psetheta_file = osp.join(TePose_DB_DIR, 'h36m_test_front_25fps_tight_pseudotheta.pt')
        if osp.isfile(psetheta_file):
            self.psetheta = joblib.load(psetheta_file)
        else:
            raise ValueError(f'{psetheta_file} do not exists')
        print(f'Loaded h36m_val dataset from {db_file}')
    ...
    def get_single_item(self, index):
        start_index = self.vid_indices[index*2]
        end_index = self.vid_indices[index*2+1]
        ...
        vidlen_each[0] = (end_index - start_index + 1).item()
